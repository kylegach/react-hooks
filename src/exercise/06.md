...
- Fetching data is always a side-effect
- asyncThing().then() vs. async/await asyncThing()
- Conditional returns
- Loading states
- Error handling
- Different ways to handle errors in a promise
- role="alert"
- Using a single status variable to conditionally return and why that's more
  clear
- Combining multiple states into one object
  - Why to consider doing so
  - Limitations and gotchas with approach
- ErrorBoundary
  - Where to place it
  - How to make it re-usable
- Using `key` to force a re-render when appropriate
- react-error-boundary lib
  - Recovering from an error using lib's reset features
  - Why using `key` can cause too many re-renders
  - `resetKeys` and how that allows "auto" recovering
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=06%3A%20useEffect%3A%20HTTP%20requests&em=kyle.gach%40gmail.com).
