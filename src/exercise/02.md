...
- Getting and setting a string in localStorage
- Initialize state with a function, so that it only runs on first render (lazy)
- Why and how to define a dependency list for useEffect calls
- Extracting re-usable functionality into custom hooks
- The "useHookName" convention, and why to use it
- Taking advantage of API familiarity
  - Signature of `useLocalStorageState` is very similar to `React.useState`
- Considerations when making a flexible custom hook API
- Dependency injection pattern
- Brief intro to React.useRef and ref usage
- Order of hook calls, especially relative to render operation
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=02%3A%20useEffect%3A%20persistent%20state&em=kyle.gach%40gmail.com).
